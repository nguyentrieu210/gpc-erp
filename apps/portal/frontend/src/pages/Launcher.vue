...
async function logout(){loggingOut.value=true;try{await fetch('/api/method/portal.api.portal_logout',{method:'GET',credentials:'include'})}catch{};document.cookie.split(";").forEach(c=>{const n=c.indexOf("=")>-1?c.substr(0,c.indexOf("=")).trim():c.trim();document.cookie=n+"=;expires=Thu,01 Jan 1970 00:00:00 GMT;path=/"});window.location.href='/portal_app/login'}
