...
<FeatherIcon name="file-plus" class="h-5 w-5 text-violet-600"/><h1 class="text-lg font-bold text-gray-900 flex-1">Hóa đơn mua (PI)</h1>
<Button variant="solid" theme="sky" @click="openCreate">+ Tạo hóa đơn</Button>
</header>
<main class="flex-1 p-4 max-w-4xl mx-auto w-full">
<div class="flex gap-2 mb-3"><select v-model="sup" @change="reload" class="inp w-auto"><option value="">Tất cả NCC</option><option v-for="s in suppliers" :key="s.name" :value="s.name">{{ s.supplier_name }}</option></select></div>
...
<div class="flex-1 min-w-0"><div class="font-medium">{{ r.name }} <span class="text-xs" :class="r.docstatus===1?'text-emerald-600':'text-amber-600'">{{ r.docstatus===1?'Đã ghi sổ':'Nháp' }}</span></div>
<div class="text-xs text-gray-500">{{ r.supplier_name }} · {{ $fmtDate(r.posting_date) }}<span v-if="r.bill_no"> · Số: {{ r.bill_no }}</span></div></div>
<div class="text-right shrink-0"><div class="font-semibold">{{ fmtVnd(r.grand_total) }}</div>
<div class="text-xs" :class="r.outstanding_amount>0?'text-red-600':'text-gray-400'">{{ r.outstanding_amount>0 ? 'Còn '+fmtVnd(r.outstanding_amount) : 'Đã trả hết' }}</div></div></div></div>
</main>
...
<h3 class="text-lg font-semibold mb-4">Hóa đơn mua</h3>
...
<label class="block"><span class="text-sm text-gray-600 block mb-1">Số hóa đơn NCC</span><input v-model="f.bill_no" class="inp"/></label></div>
...
<div class="flex justify-end gap-2 mt-5"><Button variant="subtle" @click="show=false">Hủy</Button><Button variant="solid" theme="violet" :loading="saving" @click="save">Lưu & Ghi sổ</Button></div></div></div>
</div></template>
<script setup>
...
const rows=ref([]),loading=ref(false),sup=ref('')
...
async function reload(){loading.value=true;try{const r=await callApi('muahang.api.get_purchase_invoices',{supplier:sup.value},'GET');rows.value=r?.entries||[]}finally{loading.value=false}};reload()
const show=ref(false),saving=ref(false);const f=reactive({supplier:'',bill_no:'',items:[{item_code:'',qty:1,rate:0}]})
function openCreate(){f.supplier=suppliers.value?.suppliers?.[0]?.name||'';f.bill_no='';f.items=[{item_code:'',qty:1,rate:0}];show.value=true}
async function save(){const items=f.items.filter(r=>r.item_code&&r.qty);if(!items.length||!f.supplier){alert('Chọn NCC + ít nhất 1 dòng');return};saving.value=true;try{await callApi('muahang.api.create_purchase_invoice',{supplier:f.supplier,items:JSON.stringify(items),bill_no:f.bill_no,submit:1});show.value=false;await reload()}catch(e){alert('Lỗi: '+(e?.message||e))}finally{saving.value=false}}
